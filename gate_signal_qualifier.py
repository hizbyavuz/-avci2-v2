...
VERSION="gate-signal-qualifier-v1-20260926"
PRIMARY_TARGET=10
MIN_SELECTED_N=30
MIN_BASELINE_N=30
MAX_Q=0.05
...
def main():
    ...
    with sqlite3.connect(OBS_DB,timeout=30) as obs, sqlite3.connect(VAL_DB,timeout=30) as val:
        ...
        batch=health["batch_id"]; proven=latest_proven(obs)
        ...
        for e in events:
            ...
            match=next((r for r in proven if feats.get(r["combo"])),None)
            safety="UNKNOWN"
            hard_veto=None
            ...
            if health["status"]!="VALID":
                ...
                reason="LIVE_MATCHES_VALIDATION_PROVEN_ACTIVATION"
